...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.t_step = (self.t_step + 1) % UDPATE_EVERY
        if self.t_step == 0:
    ...
    def learn(self, experiences, gamma):
        states, actions, reards, next_states, dones = experiences
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.experience = nametuple("Experience", field_name=["state", "action", "reward", "next_state", "done"])
        self.seed = random.seed(seed)
       
    
    def add(self, state, action, reward, next_state, done):
        e = self.experience(state, action, reqard, next_state, done)
